/* eslint jsx-a11y/img-has-alt: 0 */

export const ADD_BILL = 'ADD_BILL';
export const DELETE_BILL = 'DELETE_BILL';
export const MODIFY_BILL = 'MODIFY_BILL';
export const UPDATE_INCOME_AFTER_BILLS = 'UPDATE_INCOME_AFTER_BILLS';

export const ADD_ACCOUNT = 'ADD_ACCOUNT';
export const DELETE_ACCOUNT = 'DELETE_ACCOUNT';
export const MODIFY_ACCOUNT = 'MODIFY_ACCOUNT';

export const UPDATE_INCOME = 'UPDATE_INCOME';
export const UPDATE_PERCENT_REMAINING = 'UPDATE_PERCENT_REMAINING';

export const FB_REGISTER = 'FB_REGISTER';
export const FB_LOGIN = 'FB_LOGIN';
export const FB_LOGIN_SUCCESS = 'FB_LOGIN_SUCCESS';
export const FB_LOGIN_FAIL = 'FB_LOGIN_FAIL';
export const FB_GOOGLE_LOGIN = 'FB_GOOGLE_LOGIN';
export const FB_LOGOUT = 'FB_LOGOUT';
export const FB_ADD_BILL = 'FB_ADD_BILL';
export const FB_DELETE_BILL = 'FB_DELETE_BILL';
export const FB_ADD_ACCOUNT = 'FB_ADD_ACCOUNT';
export const FB_DELETE_ACCOUNT = 'FB_DELETE_ACCOUNT';
export const FB_UPDATE_INCOME = 'FB_UPDATE_INCOME';
export const FB_GET_BILLS = 'FB_GET_BILLS';
export const FB_GET_ACCOUNTS = 'FB_GET_ACCOUNTS';
export const FB_GET_INCOME = 'FB_GET_INCOME';
export const FB_INIT_USER = 'FB_INIT_USER';
export const FB_INIT_USER_SUCCESS = 'FB_INIT_USER_SUCCESS';
export const FB_INIT_USER_ERROR = 'FB_INIT_USER_ERROR';
export const FB_FULL_SAVE = 'FB_FULL_SAVE';
export const FB_FULL_SAVE_SUCCESS = 'FB_FULL_SAVE_SUCCESS';
export const FB_FULL_SAVE_ERROR = 'FB_FULL_SAVE_ERROR';
export const FB_SET_BILLS = 'FB_SET_BILLS';
export const FB_SET_ACCOUNTS = 'FB_SET_ACCOUNTS';

export const USER_RESET_STATE = 'USER_RESET_STATE';
export const FB_RESET_STATE = 'FB_RESET_STATE';
export const RESET_STATE = 'RESET_STATE';
